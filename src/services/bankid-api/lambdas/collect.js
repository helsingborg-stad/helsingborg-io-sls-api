...
export const main = async (event, context) => {
    ...
    if (isUserAgentMittHelsingborgApp(headers) && isBankidCollectStatusComplete(bankIdCollectResponse.data)) {
        await putEvent(bankIdCollectResponse.data.completionData, 'BankIdCollectComplete', 'bankId.collect');

        const personalNumber = bankIdCollectResponse.data.completionData.user.personalNumber;

        const [generateAuthorizationCodeError, authorizationCode] = await to(generateAuthorizationCode(personalNumber));
...
async function generateAuthorizationCode(personalNumber) {
    const [authorizationCodeSecretError, auhtorizationCodeSecret] = await to(
        secrets.get(CONFIG_AUTH_SECRETS_AUTHORIZATION_CODE.name, CONFIG_AUTH_SECRETS_AUTHORIZATION_CODE.keyName),
    );
