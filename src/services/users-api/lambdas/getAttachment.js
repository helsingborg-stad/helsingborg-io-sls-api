...
export async function main(event, context) {
    ...
    const [getFileError, userS3File] = await to(getFileFromUserS3Bucket(`${personalNumber}/${filename}`));
    if (getFileError) {
        log.error('Get file error', context.awsRequestId, 'service-users-api-getAttachment-001', getFileError);
