...
import { getPersonPostSoapRequestPayload, getErrorMessageFromXML, getPersonPostCollection } from '../helpers/parser';
...
async function requestNavetUser(personalNumber) {
    ...
    const [postNavetClientError, navetUser] = await to(
        request.call(navetRequestClient, 'post', ssmParams.personpostXmlEndpoint, personPostSoapRequestPayload),
    );
