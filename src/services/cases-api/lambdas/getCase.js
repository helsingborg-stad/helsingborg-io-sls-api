...
export async function main(event, context) {
    ...
    if (getUserCaseError) {
        log.error('Get user case error', context.awsRequestId, 'service-cases-api-getCase-001', getUserCaseError);
...
async function getUserCase(personalNumber, id) {
    const [getUserApplicantCaseError, userApplicantCaseResult] = await to(getUserApplicantCase(personalNumber, id));
    ...
    const [getCoApplicantCaseError, userCoApplicantCaseResult] = await to(getUserCoApplicantCase(personalNumber, id));
    ...
    const userCaseList = concatAndDeDuplicateCase(userApplicantCaseResult.Items, userCoApplicantCaseResult.Items);
