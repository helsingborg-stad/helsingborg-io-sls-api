  ...
  field.loadPrevious.forEach((matchString) => {
    ...
    if (multipleFieldRegex.test(matchString)) {
      ...
      const idRegex = new RegExp(
        `${strArray?.[0] || ''}.[0-9]+.${strArray?.[1] || ''}`
      );
      const previousAnswersMatches = previousAnswers.filter((obj) =>
        idRegex.test(obj.field.id)
      );
      if (previousAnswersMatches.length > 0) {
        previousAnswersMatches.map((answer) =>
          formatAnswer(answer.id, field.tags, answer.value)
        );
...
function getInitialValue(field, applicants, previousAnswers) {
  ...
  field.loadPrevious.forEach((matchPattern) => {
    ...
    if (applicantRoles.includes(patternParts[0])) {
      ...
      const person = applicants.find(
        (applicant) => applicant.role === applicantRole
      );
      initialValue =
        getPersonInfo(person, patternParts.slice(1)) || initialValue;
      return;
    }
  ...
  return initialValue
    ? formatAnswer(field.id, field.tags, initialValue)
    : undefined;
}
...
function populateAnswers(dataMap, applicants, previousAnswers) {
  ...
  dataMap.forEach((field) => {
    ...
    const initialFieldValue = getInitialValue(
      field,
      applicants,
      previousAnswers
    );
