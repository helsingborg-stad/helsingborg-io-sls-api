...
export async function main(event, context) {
    ...
    const userCaseListWithoutKeys = userCaseList.map((item) => objectWithoutProperties(item, ['PK', 'SK', 'GSI1']));
...
async function getUserCaseList(personalNumber) {
    const [getUserApplicantCaseListError, applicantCaseListResult] = await to(getUserApplicantCaseList(personalNumber));
    ...
    return concatAndDeDuplicateCaseList(applicantCaseListResult.Items, coApplicantCaseListResult.Items);
}
