...
export const main = async (event, context) => {
    ...
    if (parseJsonError) {
        log.warn('JSON Parse error', context.awsRequestId, 'service-auth-token-generateToken-001', parseJsonError);

        return response.failure(parseJsonError);
    }

    const [validationError, validatedEventBody] = await to(validateEventBody(parsedJson, tokenValidationSchema));

    if (validationError) {
        log.warn('Validation error', context.awsRequestId, 'service-auth-token-generateToken-002', validationError);
    ...
    const [getRefreshTokenError, refreshToken] = await to(
        generateToken(CONFIG_AUTH_SECRETS.refreshToken, personalNumber, REFRESH_TOKEN_EXPIRES_IN_MINUTES)
    );
