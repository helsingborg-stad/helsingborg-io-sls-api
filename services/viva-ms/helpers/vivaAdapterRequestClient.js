...
async function sendVivaAdapterRequest({ endpoint, method, body = undefined }) {
    ...
    const requestClient = request.requestClient({}, { 'x-api-key': xApiKeyToken }, REQUEST_TIMEOUT_IN_MS);

    const [requestError, response] = await to(request.call(requestClient, method, `${vadaUrl}/${endpoint}`, body));
