...
import { CASE_PROVIDER_VIVA, TWELVE_HOURS, VIVA_CASE_CREATED, NOT_STARTED_VIVA } from '../../../libs/constants';
...
function getCaseListOnPeriod(vivaPerson) {
    ...
    const casesQueryParams = {
        ...
        FilterExpression: 'details.period.startDate = :periodStartDate AND details.period.endDate = :periodEndDate',
...
async function createRecurringVivaCase(vivaPerson, user) {
    ...
    const applicantPersonalNumber = stripNonNumericalCharacters(String(vivaPerson.case.client.pnumber));
...
function getEncryptionAttributes(vivaPerson) {
    ...
    const mainApplicantPersonalNumber = stripNonNumericalCharacters(String(vivaPerson.case.client.pnumber));
